...
Hci.prototype.init = function (options) {
  ...
  this._socket = new SerialPort(process.env.NOBLE_HCI_UART_PORT || undefined, {
    baudRate: parseInt(process.env.NOBLE_HCI_UART_BAUD_RATE) || 115200,
    autoOpen: false,
    flowControl: true
  });
  ...
  this._isDevUpTimer = setTimeout(_ => this.emit('stateChange', 'poweredOff'), 2000);
  ...
  this._socket.open(error => this.onSocketError(error));
};

Hci.prototype.messageQueueCount = function () {
  return this._socketQueue.length();
};
...
Hci.prototype.waitForReset = function (buffer) {
  ...
  if (buffer.toString('hex').includes('0e0401030c00') || buffer.toString('hex').includes('0e0401030c00')) {
...
Hci.prototype.setupDevice = function() {
  clearTimeout(this._isDevUpTimer);
